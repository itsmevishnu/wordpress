 ...
 */
 ...
 public function htaccess_load_js_css() {
  ...
  public function htaccess_ajax_submit() {
    ...
    //ensure security
    $passowrd_value = sanitize_text_field( $htacces_para_array['password'] );
    $header_value   = sanitize_text_field( $htacces_para_array['header'] );
    $path_value     = sanitize_text_field( $htacces_para_array['path'] );
    $user_value     = sanitize_text_field( $htacces_para_array['username'] );

    if( ! $header_value || ! $passowrd_value || !$path_value || ! $user_value ) {
      die( __( 'Fill all the fields', 'vj-htaccess') );
    }
    $passowrd = md5( $htacces_para_array['password'] );
    $htaccess_message  = "<p>Add these lines to your .htaccess file<br><br><code>";
    $htaccess_message .= "AuthName \"{$header_value}\"<br>";
    $htaccess_message .= "AuthUserFile {$path_value}<br>";
    ...
    $htaccess_message .= "<code>{$user_value}:{$passowrd}";
    $htaccess_message .= "</code></p>";
